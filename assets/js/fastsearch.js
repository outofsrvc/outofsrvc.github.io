...
const searchIndex = document.querySelector('meta[name="search-index"]')?.content;
...
const renderResults = (results) => {
    if (!Array.isArray(results) || results.length === 0) {
        ...
        if (sInput.value.trim()) {
            setStatus(document.documentElement.lang === 'ar' ? 'لا توجد نتائج.' : 'No results found.');
        }
        return;
    }
    ...
    setStatus(document.documentElement.lang === 'ar'
...
const initSearch = async () => {
    ...
    try {
        if (!searchIndex) {
            throw new Error('Search index URL is missing');
        }
        ...
        if (data) {
            ...
            resList.setAttribute('aria-busy', 'false');
            setStatus('');
            sInput.focus();
        }
    } catch (error) {
        resList.setAttribute('aria-busy', 'false');
        setStatus(document.documentElement.lang === 'ar'
            ? 'تعذر تحميل البحث. حاول تحديث الصفحة.'
            : 'Search could not be loaded. Try refreshing the page.');
        console.error(error);
    }
};
...
document.addEventListener('keydown', (event) => {
    ...
    if (key === 'Escape' && (isInSearchBox || sInput?.value)) {
        reset();
        return;
    }
